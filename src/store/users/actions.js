...
export function getUsers(context) {
  ...
  axios.get(process.env.BACKEND_LOCATION + 'users', { crossdomain: true }).then(response => {
...
export async function save(context, data) {
  if (data.id === null) {
    ...
    const user = await axios.put(process.env.BACKEND_LOCATION + 'users/' + data.id, data.user)
    let media = null
    if (data.media) {
      media = await axios.put(process.env.BACKEND_LOCATION + 'users/' + data.id + '/avatar', data.media)
    }
    return {
      user: user,
      media: media
